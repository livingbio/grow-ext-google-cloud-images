...
class UploadHandler(blobstore_handlers.BlobstoreUploadHandler):
    def get(self, bucket=None):
        ...
        action = blobstore.create_upload_url("/callback", gs_bucket_name=gs_bucket_name)
        kwargs = {"bucket": gs_bucket_name, "action": action}
        self.response.out.write(template.render("upload.html", kwargs))


class RedirectHandler(webapp2.RequestHandler):
    @ndb.toplevel
    @expires(expire_interval=timedelta(days=365))
    def get(self, gs_path):
        gs_path = self.request.get("gs_path") or gs_path

        try:
            gs_uri = "/gs/{}".format(gs_path.lstrip("/"))
            flags = self.request.get("f")

            key = ndb.Key(ServedImage, gs_uri)
            path = key.get()

            if not path:
                blob_key = blobstore.create_gs_key(gs_uri)
                url = images.get_serving_url(blob_key, secure_url=True)
            ...
            url = "https://storage.googleapis.com/" + gs_path
...
class GetServingUrlHandler(webapp2.RequestHandler):
    def normalize_gs_path(self, gs_path, locale):
        ...
        try:
            ...
            if "_" in locale:
                ...
                localized_gs_path = gs_path.replace("{locale}", "_{}".format(territory))
                try:
                    stat_result = gcs.stat(localized_gs_path[3:])
                    return localized_gs_path, stat_result
                except (gcs.NotFoundError, gcs.ForbiddenError):
    ...
    def get(self, gs_path):
        ...
        try:
            ...
            image_metadata = {"height": image.height, "width": image.width}
